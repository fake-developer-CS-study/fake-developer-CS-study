...
> **포트 번호**
> TCP와 UDP는 **포트 번호**라는 숫자를 이용하여 컴퓨터 안의 어떤 서비스(애플리케이션)에게 데이터를 전달하면 좋은지를 식별합니다. 포트 번호는 0~65535(16비트 분)까지의 숫자로 되어 있으며, 범위에 따라 용도가 정해져 있습니다. **0~1023은 잘 알려진 포트(well-known port)**라고 해서 웹 서버나 메일 서버 등과 같이 일반적인 서버 소프트웨어가 클라이언트의 서비스 요청을 대기할 때 사용합니다. **1024~49151은 등록된 포트(registered port)**로, 제조업체의 독자적인 서버 소프트웨어가 클라이언트의 서비스 요청을 대기할 때 사용합니다. **49152~65535는 동적 포트(dynamic port)**로, 서버가 클라이언트를 식별하기 위해 사용합니다.
